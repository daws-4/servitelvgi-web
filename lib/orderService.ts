...
export async function syncOrderToNetuno(id: string, certificateUrlOverride?: string, sessionUser?: any, adminPhoneOverride?: string, syncMode: 'full' | 'whatsapp_only' = 'full') {
  ...
  // Use Server-Side generated image dynamically if no certificate was manually taken
  const hostUrl = process.env.NEXT_PUBLIC_SITE_URL || 
                  (process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : 'https://admin.enlaredve.com');
  const autoGeneratedImage = `${hostUrl}/api/web/orders/${id}/certificate-image`;

  const resolvedCertificateUrl = certificateUrlOverride || order.certificateUrl || autoGeneratedImage;
