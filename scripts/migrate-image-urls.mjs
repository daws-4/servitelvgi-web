 ...
 * 
 * Usage:
 *   node scripts/migrate-image-urls.mjs            # Dry-run (default)
 *   node scripts/migrate-image-urls.mjs --execute   # Apply changes
 */
...
async function main() {
  ...
  if (DRY_RUN) {
    ...
    console.log(`   node scripts/migrate-image-urls.mjs --execute`);
  } else {
    console.log(`\n✅ Migration complete!`);
  }
